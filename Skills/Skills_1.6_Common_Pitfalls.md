...
*Converted from PDF: Skills_1.6_Common_Pitfalls.pdf*
...
Introduction
You've learned the theory (Sections 1.1-1.4) and mastered the components (Section 1.5). Now

we explore what goes wrong in practice.

This section covers the 10 most common pitfalls in skill creation, drawn from real-world

### experience and Anthropic's best practices. Each pitfall includes:
...
## 1. Activation Confusion



### Model doesn't know WHEN to activate:
...
## 2. Unmaintainable Size



### Covering 6 different domains in one skill means:
...
This is attentional residue (see Section 2.3 for details)
...
<decision_criteria>
IF query slow AND table large AND index missing THEN add index ELSE IF query slow AND table

small AND query complex THEN rewrite query ELSE IF query slow AND joins present THEN
optimize joins UNLESS joins already optimized THEN check WHERE clause UNLESS WHERE
clause already optimal THEN consider materialized view BUT ONLY IF update frequency low AND

read frequency high AND storage not constrained...
</decision_criteria>
...
| Table Size | Has Index | Query Type | Action |
...
### Only success-path unload conditions:
...
If User Intent Change was first: 4. Skill checks: "User Intent Change?" → Yes! ("Actually, never

mind") 5. Skill IMMEDIATELY deactivates 6. Python help proceeds without SQL context
interference
...
## 3. Missing Unload Conditions - Skill never deactivates → Always include with User Intent
...
## 4. Over-Complicated Logic - Nested conditions unreadable → Break into phases, max 3

levels


## 5. Insufficient Examples - Only 1-2 examples → Provide 3+ covering simple, common, edge

cases


## 6. No Self-Verification - Can't check success → Include verification (Anthropic's #1
...
## 8. Ignoring User Intent Change - Not first priority → ALWAYS check User Intent Change
