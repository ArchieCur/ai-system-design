...
**Large/Reasoning Models** : Focus on Context Engineering, Avoid
...
The Problem:
LLMs read strictly left-to-right. When the model reads the first
...
The Fix:
By repeating the prompt, the second copy of the prompt acts as a
...
Second Pass (Prompt Copy 2): The model reads it again, but now it can "attend"
...
**Examples:** "Extract the invoice number from this 50-page document," "Find the
...
Latency (Speed): Surprisingly, the impact on speed is negligible. The "pre-fill"
(reading) stage of modern GPUs is highly parallelized. Reading 2x text takes
almost the same time as reading 1x text.

Cost: It doubles your Input Token cost. If you are processing a 100-page
...
Nov 2025), “Context Rot is Real. Here’s how we built memory that
learns.” Roampal Jan 2026

• Why it matters-

The Problem:
LLMs are typically presumed to process context uniformly—that is,
...
**"Just-in-Time" (JIT) Retrieval (The Anthropic/Claude Code Method)**
...
▪ Why it works:
This forces the model to move the relevant tokens from the
...
**Outcome-Based RAG (The "Roampal" Standard)**
▪ Concept: Stop ranking documents by "Semantic Similarity" (how much they look
like the query).

The Fix:
Rank documents by "Success Rate."

Mechanism:
If Document A was retrieved for a query and the user clicked
...
**Context Caching with "Decay Policies"**
▪ Concept: Treat the context window like a cache, not a log.

The Fix:
Implement a "Decay Function" for long conversations. If a piece of
...
The Problem:
LLMs are easily distracted by irrelevant context. Accuracy
...
The Fix:
Treat the context window as a database. Use techniques like asking the model to
...
-Be mindful of what you put in the context window. Treat the context window as a
...
June 2025, Technical Report)

• Why it matters-

The Problem:

The Wharton 2025 study tested Chain-of-Thought prompting
...
Models" (Zhou et al., Google Research, April 2023, [2205.10625]), "Decomposing Tasks into Atomic Steps Improves LLM Performance" (EMNLP 2025, University of Washington), "An Approach for Systematic Decomposition of Complex LLM Tasks" (Zhou T. et al, Columbia University, Oct 2025)

• Why it matters-

The Problem:
As the task complexity increases or when the individual reasoning
...
The Fix:
Decompose prompts into a chain of atomic tasks.

Instead of one prompt doing "Research, Analyze, and Summarize," use a
workflow:
...
The Problem:
Context leakage can occur when a model confuses the prompt
...
1. You can also ask the model to respond in markdown.
2. Using code blocks in a prompt is a key case for using markdown.
...
"Global", "year": 2024, "effects": ["Rising temperatures", "Melting ice caps", "Severestorms"] }
...
The Problem-
Models reasoning performance degrades because the model tries
...
Only use one-shot or few-shot prompts when output formatting matters, or
when you want the model to mimic a certain tone, structure, or behavior.

Using examples (shots) are excellent for teaching syntax (JSON, XML, Speaking
Style)
...
The Problem:
Giving a generic role to a model does not define the model’s
autonomy or probability distributions.

The Fix:
Assigning a "Senior/Expert" persona doesn't just make the model
...
The Problem:
The human capital used to create multiple versions of a prompt in
...
The Fix:
Use a "Meta-Prompt." Paste your draft prompt into a reasoning
...
think to use.
Also available are tools that automatically generate and test prompt variations  (e.g., OpenAI or Anthropics's Prompt Optimizer) to find the best-performing prompt with minimal human intervention.
...
The Problem:
Prompts do not always provide responses that are what was
...
The Fix:
Using prompt evaluations help fine-tune prompts to achieve the desired
outcomes.

In Practice: You cannot call a prompt "good" unless you have run it against 20
...
The Problem:
Models are RLHF-trained (Reinforcement Learning from Human
Feedback) to be helpful, which often makes them "yes-men."

The Fix:
Add instructions to your prompts

In Practice: There are many techniques that can be used in instructing the model
...
**The Problem:**
LLMs are increasingly used for development, but developers often mistakenly rate insecure generated code as secure.

**The Fix:**
Using a specific security prefix can reduce vulnerabilities by up to 56% in advanced models like GPT-4o. For maximum security, iterative RCI can repair between 41.9% and 68.7% of remaining flaws.

**The "Gold Standard" Workflow:**

1. **Proactive Prefix:** Always start your coding prompt with the validated security persona.
...
• This field guide was developed collaboratively between a human researcher and Claude (Anthropic's AI assistant) in February 2026.
• Human Contribution: Research synthesis, source identification, validation of techniques across multiple authoritative sources (EMNLP 2025, Google Research, Meta FAIR, OpenAI, Anthropic, Wharton, and others), curriculum design vision, and editorial direction.
