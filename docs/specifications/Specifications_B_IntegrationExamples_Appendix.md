# Specifications_B_IntegrationExamples_Appendix

*Converted from PDF: Specifications_B_IntegrationExamples_Appendix.pdf*
...
You've learned each layer individually. Now see how they work together to create clear,
actionable specifications that prevent common failures.
...
- 3D Secure: Test with high-value transaction, verify challenge appears
...
- Target devices: iPhone 11+ (3-year-old phones)
...
□ Works on older devices (iPhone 11, tested)
...
Resolution: Limit enforced (MUST), but burst makes it flexible (SHOULD), for business reasons
(CONTEXT), aligned with fairness goal (INTENT), proven working (VERIFICATION).
...
Why it fails: Can verify compliance but don't understand WHY these constraints exist or what
...
Fix: Either change MUST (custom payment) or change INTENT (prioritize compliance and time-
to-market).
