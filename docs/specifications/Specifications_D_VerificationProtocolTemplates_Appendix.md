...
Command: lighthouse https://example.com --throttling-method=simulate -- throttling.cpuSlowdownMultiplier=4 --output json --output-path ./report.json Expected:
...
• Index usage confirmed (no full table scans on large tables) Pass: All queries use indexes, execution time within threshold

Tool: Application Performance Monitoring (New Relic, DataDog) Metric: Database query duration (p95) Expected: p95 < 100ms for all queries Pass: No slow queries above threshold


4. RESOURCE UTILIZATION: Tool: Load testing with monitoring Command: artillery run load-test.yml (while monitoring resources) Monitor: CPU usage, memory usage, disk I/O
...
- No cascading failures (circuit breakers work)
 ...
 Retry logic works (exponential backoff, max retries) Pass: System handles failures gracefully
...
Monitoring and alerts in place (integration health observable) □ Documentation
