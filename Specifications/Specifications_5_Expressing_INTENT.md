# Specifications_5_Expressing_INTENT

*Converted from PDF: Specifications_5_Expressing_INTENT.pdf*
...
You've learned how to write MUST constraints (boundaries), SHOULD guidelines (preferences),
and CONTEXT (planning information). Now we explore INTENT—the "why" behind everything.


### From a model’s perspective (using intent daily):



### When you provide INTENT, you're saying:

"Here’s WHY these constraints exist, WHAT we're trying to achieve, and HOW success looks. Use

this to make intelligent decisions when the path isn't clear."
...
Result: A model can explain decisions, suggest alternatives if constraints conflict, and align with
your actual goals.
...
"The rate limit is set at 100/min to catch buggy infinite loops while allowing 2x burst headroom
for legitimate heavy use. Current data shows power users max out at 60/min.

Increasing to 200 would still catch bugs (way above normal use) but provide even more
headroom. This aligns with the intent (don't frustrate legitimate users).
...
This meets all MUSTs while respecting priority order. The 1-2 second delay on error logs is
acceptable since debugging isn't time-critical.
...
This directly supports the goal (reach $100K MRR) by maximizing time spent on revenue-driving
features.
...
• I don't know which to optimize for
...
#1 pain point with competitor. Worth the investment.
...
Suggest: Validate offline text experience in v1.0, consider video for v1.2 if users request it AND
we have resources after Android launch.
...
You now know how to express intent so models understand your goals and make aligned
decisions.
